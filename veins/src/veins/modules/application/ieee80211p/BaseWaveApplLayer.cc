...
bool compare_timestamp(const struct neighbour& first, const struct neighbour& second)
{
  return ( first.lastUpdate > second.lastUpdate );
}
...
void BaseWaveApplLayer::initialize(int stage) {
	...
	if (stage==0) {
		...
		neighbourVector.setName("#Neighbours");
		receivedMessages = 0;

		sendBeaconEvt = new cMessage("beacon evt", SEND_BEACON_EVT);
		sendMessageEvt = new cMessage("scheduleMessage", 1234);

		//simulate asynchronous channel access
		double offSet = dblrand() * (par("beaconInterval").doubleValue()/2);
		offSet = offSet + floor(offSet/0.050)*0.050;
		individualOffset = dblrand() * maxOffset;
		double scheduleBroadcast = dblrand() * 2000;
		...
		scheduleAt(simTime() + scheduleBroadcast, sendMessageEvt);
...
void BaseWaveApplLayer::handleSelfMsg(cMessage* msg) {
	switch (msg->getKind()) {
		case SEND_BEACON_EVT: {
		    ...
		    neighbourlist.sort(compare_timestamp);
		    currentTime = simTime();
		    ...
		    {
		      //  std::cerr << "List size: " << neighbourlist.size() << "Time:" << currentTime - (*it).lastUpdate << "index: " << index << std::endl;
		        if((currentTime - (*it).lastUpdate) > 5)
		        {
		            counter++;
		        }
		        index++;
		    ...
		    {
		        neighbourlist.pop_back();
		    }
		    neighbourVector.record(neighbourlist.size());
			sendWSM(prepareWSM("beacon", beaconLengthBits, type_CCH, beaconPriority, 0, -1));
			scheduleAt(simTime() + par("beaconInterval").doubleValue(), sendBeaconEvt);
			break;
		}
		case 1234:
		    WaveShortMessage* msgD = prepareWSM("Broadcast data", dataLengthBits , type_SCH, dataPriority, 0, -1);
		    msgD->setPsid(myID);
